...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .fraud-block .fraud-title { font-weight: 700; font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: #1E293B; margin-bottom: 6px; }
    .fraud-block .fraud-risk { font-weight: 700; }
    .fraud-block .fraud-risk.high { color: #DC2626; }
    .fraud-block .fraud-risk.medium { color: #D97706; }
    .fraud-block .fraud-risk.low { color: #16A34A; }
    .fraud-block .fraud-evidence { margin-top: 6px; padding-left: 12px; list-style: disc; }
...
<body>
  ...
  <script>
      ...
      try {
        ...
        if (screenAudioTracks.length > 0) {
          ...
          document.getElementById('audioStatus').innerHTML =
            ...
            '</div>' + (isBrowserTabCapture ? 'Recording tab audio...' : 'Recording system/window audio...') + '</div>';
        } else {
          try {
    ...
    function getFraudBlockHtml(fraud) {
      ...
      const score = fraud.risk_score != null ? Math.round(fraud.risk_score) : '—';
      const scamType = esc(fraud.scam_type || '—');
